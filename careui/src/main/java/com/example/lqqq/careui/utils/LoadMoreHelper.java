...
public class LoadMoreHelper extends RecyclerView.OnScrollListener {
    ...
    private RefreshLoadAdapter mRefreshLoadAdapter;
    ...
    public LoadMoreHelper(RecyclerView recyclerView, RefreshLoadAdapter refreshLoadAdapter) {
        mLayoutManager = recyclerView.getLayoutManager();
        mRefreshLoadAdapter = refreshLoadAdapter;

        if (mLayoutManager instanceof GridLayoutManager) {
            mRefreshLoadAdapter.setAdapterType(RefreshLoadAdapter.ADAPTER_TYPE_GRID);
            mRefreshLoadAdapter.setSpanCount(((GridLayoutManager) mLayoutManager).getSpanCount());
        } else if (mLayoutManager instanceof LinearLayoutManager) {
            mRefreshLoadAdapter.setAdapterType(RefreshLoadAdapter.ADAPTER_TYPE_LINEAR);
        }
        ...
        if (mIsSwipeToLoadEnabled && SCROLL_STATE_IDLE == newState && !mLoading) {
            ...
            if (mLayoutManager instanceof LinearLayoutManager) {
                ...
                if (lastCompletePosition == mLayoutManager.getItemCount() - 2) {
                    ...
                    mLoading = true;
                    mRefreshLoadAdapter.setLoadItemState(true);
                    if (mListener != null) {
                        mListener.onLoad();
                    }
                }
    ...
    public void setSwipeToLoadEnabled(boolean isSwipeToLoadEnabled) {
        if (mIsSwipeToLoadEnabled != isSwipeToLoadEnabled) {
            ...
            mRefreshLoadAdapter.setLoadItemVisibility(isSwipeToLoadEnabled);
        }
    }
    ...
    public void setLoadMoreFinish() {
        mLoading = false;
        mRefreshLoadAdapter.setLoadItemState(false);
    }
